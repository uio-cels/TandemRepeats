...
linMixEffNlmePheno <- function(startpos){
  ...
  #dimnames(v$coefficients)[[1]][2] <- base::colnames(startpos[i]) #change the variable names
  #v$methTitle <- paste(v$methTitle, "-- for gene:",unique(startpos$GENE), "-start:", unique(startpos$START))
